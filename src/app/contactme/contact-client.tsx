...
function ContactForm() {
  ...
  // #40: inline, non-blocking feedback. This used to be a series of alert()
  // dialogs - unstyled, modal, and invisible to anything reading the page.
  ...
  const handleChange = (e: { target: { name: string; value: string } }) => {
    ...
    // Clear stale feedback as soon as the visitor starts correcting things.
    setStatus(null);
  };
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.success) {
        setStatus({ kind: "ok", message: "Thanks - your message has been sent." });
    ...
    <NeonGradientCard className="h-[fit-content] max-w-sm items-center justify-center text-center ">
      ...
      <CardContent>
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          {status && (
            // role=alert for failures so it is announced immediately;
            // role=status for success, which is polite.
            <p
